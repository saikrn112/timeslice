...
env "${EXTRA[@]+"${EXTRA[@]}"}" \
  ...
  TIMESLICE_PLANNER_INTENDED="${INTENDED:-0}" \
  TIMESLICE_SCROLL="${SCROLL:-top}" \
  TIMESLICE_PLANNER_METHOD="${METHOD:-}" \
  TIMESLICE_DORMANT_DAYS="${DORMANT:-}" \
  TIMESLICE_OPEN_SETTINGS="${SETTINGS:-0}" \
  TIMESLICE_SCOPE="${SCOPE:-}" \
  TIMESLICE_MONTH_RESOLUTION="${RES:-}" \
  TIMESLICE_SYNC_WARN="${SYNC_WARN:-0}" \
  TIMESLICE_OPEN_ALLOCATIONS="${ALLOC:-0}" \
