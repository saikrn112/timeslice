...
// MARK: - Capacity (committed vs free hours)

/// The de-duplication rule is the whole point of this type, so it's the bulk of what's checked.
///
/// The reported gap was "I don't understand this distribution at the top — where is it free from, still
/// to do from". Nothing showed it: the hero printed `tracked ÷ waking` as one percentage with its
/// denominator invisible. Adding the parts up is easy; adding them up WITHOUT double-booking an hour
/// across a tag, its project and its task is not, and that's what would quietly make "committed" exceed
/// the hours in a day.
func testCapacity() throws {
    ...
    // Scopes that genuinely NEST, which the dedup rule needs to be exercised at all: taskA ⊂ Timeslice
    // {alpha, gamma} ⊂ Deep Work {alpha, gamma, beta}. A tag covering exactly one project would give the
    // tag and the project an identical task set — a real case, pinned separately below, but it tests the
    // equality path rather than containment.
    let groupID = try store.upsertTaskProject(name: "Timeslice", colorHex: "#76B7B2")
    let taskA = try store.createProject(name: "alpha", colorHex: "#4E79A7", inGroup: groupID)
    _ = try store.createProject(name: "gamma", colorHex: "#B07AA1", inGroup: groupID)
    let sideID = try store.upsertTaskProject(name: "Side", colorHex: "#E15759")
    _ = try store.createProject(name: "beta", colorHex: "#F28E2B", inGroup: sideID)
    let tagID = try store.upsertTag(name: "Deep Work", colorHex: "#59A14F")
    try store.addTag(tagID, to: .project(groupID))
    try store.addTag(tagID, to: .project(sideID))
    ...
    // A Wednesday midday, and a WEEK range so weekly targets aren't pro-rated — the dedup arithmetic
    // is easier to read at 1:1, and pro-rating is asserted separately below.
    let now = date(2026, 3, 11, 12, 0)
    let week = DateRange.resolve(unit: .week, anchor: now, calendar: cal)
    let waking: TimeInterval = 16 * 3600

    func rows(_ range: DateRange, _ intervals: [Interval]) throws -> [BudgetRows.Row] {
        BudgetRows.build(targets: try store.listTargets(), tasks: tasks, groups: groups, tags: tags,
                         tagIDsByTask: tagIDsByTask, intervals: intervals, viewedRange: range, now: now)
    }

    do { // capacity is the setting times the range's DAYS, counted by calendar
        check(approx(Capacity.days(in: week, calendar: cal), 7, 0.001), "a week is seven days")
        let day = DateRange.resolve(unit: .day, anchor: now, calendar: cal)
        check(approx(Capacity.days(in: day, calendar: cal), 1, 0.001), "a day is one day")
        // 8 March 2026 is the US spring-forward Sunday: that week is 167 hours, and dividing by 86,400
        // would give 6.96 days — the bug class this project bans from every other total.
        let dstWeek = DateRange.resolve(unit: .week, anchor: date(2026, 3, 8, 12, 0), calendar: cal)
        check(approx(Capacity.days(in: dstWeek, calendar: cal), 7, 0.001),
              "the spring-forward week is still seven days, not 6.96")
    }

    do { // the headline case from the design mockup: overlapping scopes must not double-book
        _ = try store.setTarget(subject: .tag(tagID), seconds: 10 * 3600,
                                direction: .atLeast, period: .week)
        _ = try store.setTarget(subject: .project(groupID), seconds: 6 * 3600,
                                direction: .atLeast, period: .week)

        let b = Capacity.breakdown(rows: try rows(week, []), wakingSecondsPerDay: waking,
                                   range: week, trackedSeconds: 0, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        // The project's 6h are PART of the tag's 10h. Ten hours of intent, not sixteen.
        check(approx(b.owedSeconds, 10 * 3600, 1),
              "a tag ≥10h containing a project ≥6h owes 10h, not 16h — got \(b.owedSeconds / 3600)h")
        check(b.owed.count == 2, "both commitments still appear by name")
        let byName = Dictionary(uniqueKeysWithValues: b.owed.map { ($0.name, $0.seconds) })
        check(approx(byName["Timeslice"] ?? 0, 6 * 3600, 1), "the narrower commitment keeps its full 6h")
        check(approx(byName["Deep Work"] ?? 0, 4 * 3600, 1),
              "the wider one contributes only its remainder, 4h")
        check(b.owed.first?.name == "Timeslice", "largest contribution first")
    }

    do { // time already tracked reduces what's owed, and lands in `tracked` rather than vanishing
        try store.insertClosedInterval(projectID: taskA, start: date(2026, 3, 11, 9, 0),
                                       end: date(2026, 3, 11, 11, 0))
        let intervals = try store.intervals()
        let b = Capacity.breakdown(rows: try rows(week, intervals), wakingSecondsPerDay: waking,
                                   range: week, trackedSeconds: 2 * 3600, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        // Both targets saw the same 2h (the task is in the project, which carries the tag), so the tag
        // needs 8h more and the project 4h — of which 4h must be that project. Total still 8h.
        check(approx(b.owedSeconds, 8 * 3600, 1),
              "2h done against both leaves 8h owed — got \(b.owedSeconds / 3600)h")
        check(approx(b.trackedSeconds, 2 * 3600, 1), "tracked is what the caller measured")
        // 112h waking in the week, 2h spent, 8h promised.
        check(approx(b.capacitySeconds, 112 * 3600, 1), "capacity is waking × days")
        check(approx(b.freeSeconds, 102 * 3600, 1), "free is what's left after both")
        check(!b.isOvercommitted, "10h of 112h is not overcommitted")
        check(approx(b.trackedFraction + b.owedFraction + b.freeFraction, 1, 0.0001),
              "the three fractions fill the bar exactly")
    }

    do { // two targets over the SAME tasks describe the same hours: the larger binds, they don't add
        //
        // A tag on exactly one project resolves to that project's task set, so containment can't
        // separate them. Summing gave 16h for what is really 10h of intent — the case that made the
        // rule need an equality branch as well as a containment one.
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        let soloTag = try store.upsertTag(name: "Solo", colorHex: "#9C755F")
        try store.addTag(soloTag, to: .project(sideID))
        _ = try store.setTarget(subject: .tag(soloTag), seconds: 10 * 3600,
                                direction: .atLeast, period: .week)
        _ = try store.setTarget(subject: .project(sideID), seconds: 6 * 3600,
                                direction: .atLeast, period: .week)

        let fresh = try store.effectiveTagIDsByTask()
        let built = BudgetRows.build(targets: try store.listTargets(), tasks: tasks, groups: groups,
                                     tags: try store.listTags(), tagIDsByTask: fresh,
                                     intervals: [], viewedRange: week, now: now)
        let b = Capacity.breakdown(rows: built, wakingSecondsPerDay: waking, range: week,
                                   trackedSeconds: 0, tasks: tasks, tagIDsByTask: fresh, calendar: cal)
        check(approx(b.owedSeconds, 10 * 3600, 1),
              "identical scopes take the larger, not the sum — got \(b.owedSeconds / 3600)h")
        check(b.owed.count == 1, "and only the binding one is named")
        try store.deleteTag(id: soloTag)
    }

    do { // a chain of three scopes subtracts only the MAXIMAL descendants
        //
        // task ⊂ project ⊂ tag. Subtracting both the project and the task from the tag would remove the
        // task's hours twice and understate what's owed — the bug that made the rule worth a comment.
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        _ = try store.setTarget(subject: .tag(tagID), seconds: 10 * 3600,
                                direction: .atLeast, period: .week)
        _ = try store.setTarget(subject: .project(groupID), seconds: 6 * 3600,
                                direction: .atLeast, period: .week)
        _ = try store.setTarget(subject: .task(taskA), seconds: 3 * 3600,
                                direction: .atLeast, period: .week)

        let b = Capacity.breakdown(rows: try rows(week, []), wakingSecondsPerDay: waking,
                                   range: week, trackedSeconds: 0, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        // tag 10 − project 6 = 4; project 6 − task 3 = 3; task 3. Total 10h, the widest intent.
        check(approx(b.owedSeconds, 10 * 3600, 1),
              "a three-deep chain still owes the outermost 10h — got \(b.owedSeconds / 3600)h")
    }

    do { // ceilings are limits, not obligations
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        _ = try store.setTarget(subject: .task(taskA), seconds: 2 * 3600,
                                direction: .atMost, period: .week)
        let b = Capacity.breakdown(rows: try rows(week, []), wakingSecondsPerDay: waking,
                                   range: week, trackedSeconds: 0, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        check(approx(b.owedSeconds, 0, 1), "a ceiling owes nothing")
        check(b.limits.count == 1, "but it is reported as a limit")
        check(b.owed.isEmpty, "and never as a commitment")
    }

    do { // a met floor drops out entirely rather than contributing zero
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        _ = try store.setTarget(subject: .task(taskA), seconds: 1 * 3600,
                                direction: .atLeast, period: .week)
        let intervals = try store.intervals()          // already holds 2h on alpha
        let b = Capacity.breakdown(rows: try rows(week, intervals), wakingSecondsPerDay: waking,
                                   range: week, trackedSeconds: 2 * 3600, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        check(b.owed.isEmpty, "a floor already met is not listed")
        check(approx(b.owedSeconds, 0, 1), "and owes nothing")
    }

    do { // promises that don't fit are SAID to not fit, not clamped into looking fine
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        _ = try store.setTarget(subject: .task(taskA), seconds: 20 * 3600,
                                direction: .atLeast, period: .day)
        let day = DateRange.resolve(unit: .day, anchor: now, calendar: cal)
        let b = Capacity.breakdown(rows: try rows(day, []), wakingSecondsPerDay: waking,
                                   range: day, trackedSeconds: 0, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        check(b.isOvercommitted, "20h of floors in a 16h waking day is overcommitted")
        check(approx(b.freeSeconds, 0, 1), "free floors at zero rather than going negative")
        check(b.freeFraction >= 0, "and the bar never asks to draw a negative segment")
    }

    do { // a weekly target read on a DAY contributes its daily slice, not its whole week
        for t in try store.listTargets() { try store.deleteTarget(id: t.id) }
        _ = try store.setTarget(subject: .task(taskA), seconds: 7 * 3600,
                                direction: .atLeast, period: .week)
        let day = DateRange.resolve(unit: .day, anchor: date(2026, 3, 12, 12, 0), calendar: cal)
        let b = Capacity.breakdown(rows: try rows(day, []), wakingSecondsPerDay: waking,
                                   range: day, trackedSeconds: 0, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        check(approx(b.owedSeconds, 3600, 60),
              "7h/week owes ~1h on a single day — got \(b.owedSeconds / 3600)h")
    }

    do { // zero waking hours must not produce NaN fractions
        let b = Capacity.breakdown(rows: [], wakingSecondsPerDay: 0, range: week,
                                   trackedSeconds: 3600, tasks: tasks,
                                   tagIDsByTask: tagIDsByTask, calendar: cal)
        check(b.trackedFraction == 0 && b.owedFraction == 0 && b.freeFraction == 0,
              "no capacity yields zero fractions, not NaN")
    }
}
